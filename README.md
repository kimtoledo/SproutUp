...
The first authentication foundation is implemented: database-backed Better Auth sessions, canonical staff roles and capability permissions, fail-closed server-resolved session context, database-backed auth throttling, and append-only audit storage. Separate admin, borrower, and investor account/credential/session relations plus a globally unique email registry are now implemented as the forward-only identity-isolation foundation; runtime migration off the legacy unified `users` boundary remains in progress. See [`docs/IDENTITY.md`](./docs/IDENTITY.md) and [`docs/SECURITY.md`](./docs/SECURITY.md) for the cutover and release blockers.
