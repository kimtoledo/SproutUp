...
- [ ] [02 — Authentication, RBAC & Audit](./02-auth-rbac-audit.md) — existing password/session boundary, access catalogue, own-session UI, staff RBAC, immutable audit, dual-controlled role changes, and role-approvals UI implemented; separate admin/borrower/investor auth relations, global email ownership, deterministic preflight, and exact legacy account/credential/session backfill implemented; runtime/foreign-key cutover, recovery/MFA, and remaining controls remain
...
- [ ] [21 — Borrower & Investor Portal Journeys](./21-borrower-investor-portal-journeys.md) — SproutUp Tailwind design system + accessible component kit + installable PWA shell (manifest, service worker, `/offline`) established and the landing/auth surfaces migrated onto it; public auth entry plus permission-driven onboarding actions and immutable case timeline UI implemented; profile/evidence screens, portal/admin kit migration, and complete end-to-end journeys remain
