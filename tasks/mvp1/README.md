...
- [ ] [02 — Authentication, RBAC & Audit](./02-auth-rbac-audit.md) — existing password/session boundary, access catalogue, own-session UI, staff RBAC, immutable audit, dual-controlled role changes, and role-approvals UI implemented; separate account relations, global email ownership, exact backfill, and the isolated admin runtime/cookie/context are implemented; borrower/investor runtime, foreign-key cutover, recovery/MFA, and remaining controls remain
