...
- **2026-08-30 — Effective-dated configuration primitive (slice S0.2):** Added generated migration `0015_wise_lockjaw.sql` (`rule_sets` catalogue + `rule_versions`) and custom invariant migration `0016_config-rule-immutability.sql` (append-only `rule_versions`; permanent, rename-proof `rule_sets` keys; `jsonb` object-only bodies; one version per key per effective instant). Added `apps/api/src/config/rule-service.ts` — `registerRuleSet`, transaction-aware `publish` (monotonic per-key version, `unknown_rule_key` / `effective_from_conflict`, `config_rule.published` audit without the body), `resolve(key, at)` (greatest `effective_from <= at`), `listVersions`. Startup readiness now requires both relations. No rule bodies are seeded and no HTTP route is exposed — KYC field/document matrices, tax rates, investment limits, SLA thresholds, and scorecard weights are published by their owning tasks, each flagged `ASSUMED FOR PILOT`. See [`../../docs/CONFIG.md`](../../docs/CONFIG.md).
