...
Migrations `0013_robust_corsair.sql` and `0014_consent-evidence-invariants.sql` add immutable versioned consent content and exact per-user acceptance evidence. Read [CONSENTS.md](./CONSENTS.md). No content is seeded and no route or service is authorized yet; publication, active-version selection, acceptance, re-consent, withdrawal, and retention require their documented policy boundaries first.
