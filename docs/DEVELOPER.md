...
The portal also loads `/v1/sessions` for every authenticated account and lists each active session's IP address, user agent, created/expiry time, and whether it is the current device. Any session other than the current one can be revoked via `DELETE /v1/sessions/:sessionId`; the current session has no revoke control there since the header's own "Sign out" already covers it.
...
Migrations `0017_salty_molten_man.sql` and `0018_document-version-immutability.sql` add the private document store — `documents` and `document_versions`. Read [DOCUMENTS.md](./DOCUMENTS.md). A version's identity/content/provenance columns are immutable; only `scan_state` (`pending → clean/infected/error`), `scanned_at`, and `retention_until` may change; versions cannot be deleted or truncated. `apps/api/src/storage/` holds the `FileStorage` port with in-memory (tests) and local-filesystem (dev) implementations; `apps/api/src/documents/document-service.ts` handles create/add-version/scan-result/download/list with atomic metadata + audit writes and orphaned-object cleanup. Byte storage is keyed by an opaque generated id, content type is allowlisted (`application/pdf`, `image/jpeg`, `image/png`), the size cap defaults to 20 MiB, download is API-mediated and hard-gated on `scan_state = 'clean'`, and there is no public or signed object URL. No HTTP upload/download route and no `documents.*` capability exist yet — they land with the borrower KYB form.

Migrations `0015_wise_lockjaw.sql` and `0016_config-rule-immutability.sql` add the effective-dated configuration primitive — `rule_sets` (catalogue of keys) and append-only `rule_versions` (immutable, effective-dated `jsonb` bodies). Read [CONFIG.md](./CONFIG.md). `apps/api/src/config/rule-service.ts` provides `registerRuleSet`, `publish` (append the next version; `unknown_rule_key` / `effective_from_conflict`, audited as `config_rule.published` without the body), `resolve(key, at)` (the version in force at `at`, or `null`), and `listVersions`. No rule bodies are seeded and no HTTP route is exposed; KYC field matrices, tax rates, limits, SLA thresholds, and scorecard weights are published by their owning MVP task, each flagged `ASSUMED FOR PILOT`. Consumers must call `resolve` with the transaction's effective date (never `now()` for a historical recompute) and treat a `null` result as a configuration error, not a default.
