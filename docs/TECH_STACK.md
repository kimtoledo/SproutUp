...
9. **Portal identity isolation:** `admin_accounts`, `borrower_accounts`, and `investor_accounts` have separate Better Auth credential, session, verification, and rate-limit relations. A protected global email registry prevents an email or account ID from crossing portal classes and anchors mixed actor/owner attribution. Administrator runtime, staff RBAC/approval/reviewer foreign keys, and customer ownership foreign keys are active; borrower/investor runtime remains a release blocker. Borrower and investor are account types, not RBAC roles. See [`IDENTITY.md`](./IDENTITY.md).
