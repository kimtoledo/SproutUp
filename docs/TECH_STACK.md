...
9. **Portal identity isolation:** `admin_accounts`, `borrower_accounts`, and `investor_accounts` have separate Better Auth credential, session, verification, and rate-limit relations. A protected global email registry prevents an email or account ID from crossing portal classes. The administrator runtime and `admin_role_grants`/approval/reviewer foreign keys are active; borrower/investor runtime and customer ownership cutover from the legacy unified identity remain release blockers. Borrower and investor are account types, not RBAC roles. See [`IDENTITY.md`](./IDENTITY.md).
