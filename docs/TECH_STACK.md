...
9. **Portal identity isolation:** `admin_accounts`, `borrower_accounts`, and `investor_accounts` have separate Better Auth credential, session, verification, and rate-limit relations. A protected global email registry prevents an email or account ID from crossing portal classes. The administrator runtime is active on its own route/cookie/session context; borrower/investor runtime and remaining foreign-key cutover from the legacy unified identity remain release blockers. Borrower and investor are account types, not RBAC roles. See [`IDENTITY.md`](./IDENTITY.md).
