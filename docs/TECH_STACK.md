...
| Authentication | Better Auth with Drizzle adapter behind portal-specific auth-service boundaries | `apps/api/src/auth` | Admin, borrower, and investor are separate account/credential/session namespaces with globally unique email identity. Only staff use RBAC; account ownership and active operating context are resolved server-side. Keep each boundary replaceable. |
...
9. **Portal identity isolation:** `admin_accounts`, `borrower_accounts`, and `investor_accounts` have separate Better Auth credential, session, verification, rate-limit, route, cookie, and session-context boundaries. A protected global email registry prevents an email or account ID from crossing portal classes and anchors mixed actor/owner attribution. Administrator RBAC is staff-only; borrower and investor receive fixed account-class capabilities and are not RBAC roles. Migration `0024` retires the legacy unified customer auth namespace after exact reconciliation. See [`IDENTITY.md`](./IDENTITY.md).
