...
4. **Durable background work:** PostgreSQL `background_jobs` and `background_job_attempts` now provide the provider-neutral idempotency, lease, retry-budget, scheduling, terminal-state, and attempt-evidence foundation. Domain state and its job must commit transactionally. Worker/service topology and any external queue/scheduler provider remain open infrastructure decisions.
