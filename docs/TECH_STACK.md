...
9. **Portal identity isolation:** `admin_accounts`, `borrower_accounts`, and `investor_accounts` have separate Better Auth credential, session, verification, and rate-limit relations. A protected global email registry prevents an email or account ID from crossing portal classes. Borrower and investor are account types, not RBAC roles; the runtime/data cutover from the legacy unified identity remains a release blocker. See [`IDENTITY.md`](./IDENTITY.md).
