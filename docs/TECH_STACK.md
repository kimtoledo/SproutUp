...
2. **Append-only ledgers:** `ledger_accounts`, immutable `ledger_transactions`, and immutable positive debit/credit `ledger_entries` are implemented with PHP-only `numeric(30,2)`, unique posting/reversal identities, and deferred database balance enforcement. The API posting primitive adds exact preflight balance, canonical payload hashing, active-account locks, exact-retry semantics, and atomic audit evidence. A mutable wallet balance is never the source of truth. Full reversal, the chart, domain posting rules, holds, and reconciliation mappings remain task-owned.
