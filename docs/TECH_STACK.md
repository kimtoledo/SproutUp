...
2. **Append-only ledgers:** `ledger_accounts`, immutable `ledger_transactions`, and immutable positive debit/credit `ledger_entries` are implemented with PHP-only `numeric(30,2)`, unique posting/reversal identities, and deferred database balance enforcement. A mutable wallet balance is never the source of truth. The chart, domain posting rules, holds, and reconciliation mappings remain task-owned.
