...
The portal's active-sessions list and revoke control use the existing `sessions.read_own`/`sessions.revoke_own` boundary, which is scoped to the authenticated user's own sessions only — there is no client-supplied user or session-owner parameter. The current session is never offered a revoke control in that list, so a user cannot accidentally end the session they are viewing the page from other than through the dedicated "Sign out" action.
